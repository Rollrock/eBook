//
//  CommData.h
//  alarm
//
//  Created by zhuang chaoxiao on 15-6-15.
//  Copyright (c) 2015年 zhuang chaoxiao. All rights reserved.
//

#ifndef alarm_CommData_h
#define alarm_CommData_h

#import "JSONKit.h"
#import "StructInfo.h"
#import "GlobalSetting.h"

#define BASE_URL  @"http://www.hushup.com.cn/eBook/"

#define NAV_BAR_HIEGHT   30.0f

#define COMMON_BG_COLOR  [UIColor colorWithRed:34/255.0 green:168/255.0 blue:109/255.0 alpha:1]

#define SHARE_TEXT @"不起床根本就停不下来啊，这破闹钟  https://itunes.apple.com/us/app/bi-qi-nao-zhong-bu-qi-chuang/id1009624896?l=zh&ls=1&mt=8"
#define SHARE_URL  @"https://itunes.apple.com/us/app/bi-qi-nao-zhong-bu-qi-chuang/id1009624896?l=zh&ls=1&mt=8"
#define SHARE_IMAGE  ([UIImage imageNamed:@"120"])

#define UM_SHARE_KEY  @"55b06037e0f55a4422004da7"


#define BAIDU_APP_ID @"af3eb27c"
#define BAIDU_ADV_ID @"2080903"


//刷新主页书架用
#define NOTI_REFRESH_BOOK_SHELF  @"NOTI_REFRESH_BOOK_SHELF"

#define NOTI_REFRESH_BOOK_DETAIL_FONT  @"NOTI_REFRESH_BOOK_DETAIL_FONT"
#define NOTI_REFRESH_BOOK_DETAIL_COLOR  @"NOTI_REFRESH_BOOK_DETAIL_COLOR"

#endif
